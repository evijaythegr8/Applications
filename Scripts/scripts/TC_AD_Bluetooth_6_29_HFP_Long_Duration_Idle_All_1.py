...
class TC_AD_Bluetooth_6_29_HFP_Long_Duration_Idle_All_1(BluetoothTestCase):

    def execute(self):
        ComponentOne = '1'
        ComponentTwo = '2'
        handsfreeAddress = Session.getSetupFileParam('bt', 'addressheadset')
        duration = int(Session.getSetupFileParam('bt', 'hfplongduration'))*60
        ...
        # Verify that call can be made before
        ...
        time.sleep(3)
        BluetoothDialogInput("Verify that ring tone can be heard in the headset", option="TEST")
        self.SendCommand(ComponentOne, OpCodes.ANSWER_CALL, message='Answer Call')
        BluetoothDialogInput("Verify that the call is setup correctly", option="TEST")
        ...
        for i in range(0,duration/interval):
            self.log.info("Time: %s of %s"%(i*interval, duration))
            self.log.info("%s of %s"%(i*interval, duration))
        ...
        if HandsfreeConnected == "false":
            self.log.info("Handsfree not connected")
            raise Failure('Handsfree not connected')
        BluetoothDialogInput("A call will now be made. Press OK")

        # Verify that call can be made after
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_6_29_HFP_Long_Duration_Idle_All_1("TC_AD_Bluetooth_6_29_HFP_Long_Duration_Idle_All_1", duts)
        tc.run()
