...
class TC_AD_Bluetooth_11_23_AVRCP_Consecutive_Pause_Resume_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        #repeat suspend and resume
        for i in range(0,iterations):
            self.log.info('Iteration: %s of %s'%(i+1,iterations))
            self.log.info('Iteration %s'%(i+1))
            #Suspend streaming audio
            time.sleep(2)
            BluetoothDialogInput('Press Pause on A2DP Headset and verify that audio is paused. Yes/No', option="TEST")
            time.sleep(3)
            returnData = self.SendAndWaitForEvent(ComponentOne, OpCodes.GET_IS_AUDIO_PLAYING, OpCodes.GET_IS_AUDIO_PLAYING, message='Is Audio Playing app 1', timeout=30)
            if returnData == "true":
            ...
            time.sleep(2)
            BluetoothDialogInput('Press Play on A2DP Headset and verify that audio is played. Yes/No', option="TEST")
            time.sleep(3)
            returnData = self.SendAndWaitForEvent(ComponentOne, OpCodes.GET_IS_AUDIO_PLAYING, OpCodes.GET_IS_AUDIO_PLAYING, message='Is Audio Playing app 1', timeout=30)
            if returnData == "false":
                self.log.info("Audio is not playing")
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_11_23_AVRCP_Consecutive_Pause_Resume_All_1("TC_AD_Bluetooth_11_23_AVRCP_Consecutive_Pause_Resume_All_1", duts)
        tc.run()
