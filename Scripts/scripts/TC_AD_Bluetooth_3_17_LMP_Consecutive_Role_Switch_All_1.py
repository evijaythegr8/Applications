...
class TC_AD_Bluetooth_3_17_LMP_Consecutive_Role_Switch_All_1(BluetoothTestCase):

    def execute(self):
        ComponentOne = '1'
        ComponentTwo = '2'
        iterations = int(Session.getSetupFileParam('bt', 'lmpiterations'))
        ...
        outData = self.RunShellCommandAndHciDump(ComponentOne, 'hcitool lp %s RSWITCH'%deviceTwoAddress)
        self.log.info("------ Running Shell Command Done ------")

        #Link policies
        self.log.info("------ Running Shell Command Start ------")
        outData = self.RunShellCommandAndHciDump(ComponentTwo, 'hcitool lp %s RSWITCH'%deviceOneAddress)
        self.log.info("------ Running Shell Command Done ------")
        ...
        for i in range(0,iterations):
            ...
            #Switch role
            self.log.info("------ Running Shell Command Start ------")
            outData = self.RunShellCommandAndHciDump(ComponentTwo, 'hcitool sr %s slave'%deviceOneAddress)
            if find(outData,'> HCI Event: Role Change (0x12) plen 8') != -1 and find(outData,' role 0x01') != -1 and find(outData,'Role: Slave') != -1:
                self.log.info("Found correct event")
            else:
                raise Failure("Failed to set role slave")
            self.log.info("------ Running Shell Command Done ------")
            time.sleep(2)

            #Switch role
            self.log.info("------ Running Shell Command Start ------")
            outData = self.RunShellCommandAndHciDump(ComponentTwo, 'hcitool sr %s master'%deviceOneAddress)
            if find(outData,'> HCI Event: Role Change (0x12) plen 8') != -1 and find(outData,' role 0x00') != -1 and find(outData,'Role: Master') != -1:
                self.log.info("Found correct event")
            else:
                raise Failure("Failed to set role master")
            self.log.info("------ Running Shell Command Done ------")
            time.sleep(2)
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_3_17_LMP_Consecutive_Role_Switch_All_1("TC_AD_Bluetooth_3_17_LMP_Consecutive_Role_Switch_All_1", duts)
        tc.run()
