...
class TC_AD_Bluetooth_4_18_A2DP_Transfer_Audio_Stream_Source_Initiated_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        BluetoothDialogInput("Verify that the audio is heard in the A2DP headset. Yes/No", option='TEST')

        #Transfer Audio to the local speaker
        time.sleep(2)
        self.SendAndWaitForEvent(ComponentOne, OpCodes.DISCONNECT_SINK, OpCodes.DISCONNECT_SINK, message=handsfreeAddress, timeout=30)
        time.sleep(3)
        returnData = self.SendAndWaitForEvent(ComponentOne, OpCodes.IS_AUDIO_ROUTED_TO_A2DP, OpCodes.IS_AUDIO_ROUTED_TO_A2DP, message='Audio routed to A2DP?', timeout=30)
        if returnData == "true":
            self.log.info("Audio is still in A2DP")
            raise Failure('Audio is still in A2DP')
        BluetoothDialogInput("Verify that the audio is transferred to the device speakers. Yes/No", option='TEST')

        #Transfer Audio to the A2DP headset
        time.sleep(2)
        self.SendAndWaitForEvent(ComponentOne, OpCodes.CONNECT_SINK, OpCodes.CONNECT_SINK, message=handsfreeAddress, timeout=30)
        time.sleep(11)
        returnData = self.SendAndWaitForEvent(ComponentOne, OpCodes.IS_AUDIO_ROUTED_TO_A2DP, OpCodes.IS_AUDIO_ROUTED_TO_A2DP, message='Audio routed to A2DP?', timeout=30)
        if returnData == "false":
            self.log.info("Audio is still in SPEAKER")
            raise Failure('Audio is still in SPEAKER')
        BluetoothDialogInput("Verify that the audio is heard in the A2DP headset. Yes/No", option='TEST')
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_4_18_A2DP_Transfer_Audio_Stream_Source_Initiated_All_1("TC_AD_Bluetooth_4_18_A2DP_Transfer_Audio_Stream_Source_Initiated_All_1", duts)
        tc.run()
