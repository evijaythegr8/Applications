...
class TC_AD_Bluetooth_17_33_HFP_WBS_Switch_WB_NB_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        try:
            ...
            time.sleep(10)
            self.SendAndWaitForEvent(ComponentOne, OpCodes.CONNECT_HEADSET, OpCodes.CONNECT_HEADSET, message=WBShandsfreeAddress, timeout=30)
            time.sleep(12) #Remove when 10s connect delay is solved in headset
            time.sleep(4)
            HandsfreeConnected = self.SendAndWaitForEvent(ComponentOne, OpCodes.GET_IS_HEADSET_CONNECTED, OpCodes.GET_IS_HEADSET_CONNECTED, message=WBShandsfreeAddress, timeout=10)
        ...
        BluetoothDialogInput("Press Enter then turn off WBS Headset and trigger connect from Normal Headset. Then press Enter")

        #Check if NB is setup
        if self.CheckIfWB(ComponentOne, 120):
            self.log.info("WB setup")
            raise Failure('WB setup')

        BluetoothDialogInput("Verify that the audio is routed to Headset. Yes/No", option="TEST")
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_17_33_HFP_WBS_Switch_WB_NB_All_1("TC_AD_Bluetooth_17_33_HFP_WBS_Switch_WB_NB_All_1", duts)
        tc.run()
