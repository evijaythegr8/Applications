...
class TC_AD_Bluetooth_6_14_HFP_Number_Forwarding_To_HF_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        numberTwo = Session.getSetupFileParam('DUT2', 'phonenumber')
        ...
        #Check for phonenumber in logs
        print "------ Running Shell Command Start ------"
        outData = self.RunShellCommandAndHciDump(ComponentOne, 'date')
        if find(outData,numberTwo) != -1:
            print "Found correct event"
        else:
            raise Failure("phonenumber not found in log.")
        print "------ Running Shell Command Done ------"

        BluetoothDialogInput("Verify that the phone number %s can be seen in the display on the headset. Yes/No"%numberTwo, option="TEST")

        self.SendCommand(ComponentTwo, OpCodes.END_CALL, message=number)
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_6_14_HFP_Number_Forwarding_To_HF_All_1("TC_AD_Bluetooth_6_14_HFP_Number_Forwarding_To_HF_All_1", duts)
        tc.run()
