...
class TC_AD_Bluetooth_1_30_GAP_General_Incoming_Bonding_Display_Yes_No_Rejected_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        try:
            self.SendAndWaitForEvent(ComponentOne, OpCodes.SET_PAIRING_CONFIRMATION, OpCodes.UNBOND_REASON_AUTH_FAILED, message='%s;false'%deviceTwoAddress, timeout=20)
        ...
        #Try to close the link that is not up, cleaning
        try:
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_1_30_GAP_General_Incoming_Bonding_Display_Yes_No_Rejected_All_1("TC_AD_Bluetooth_1_30_GAP_General_Incoming_Bonding_Display_Yes_No_Rejected_All_1", duts)
        tc.run()
