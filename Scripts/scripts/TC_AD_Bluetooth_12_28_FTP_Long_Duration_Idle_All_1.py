...
class TC_AD_Bluetooth_12_28_FTP_Long_Duration_Idle_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        self.log.info('Waiting for Duration:  %s'%Duration)
        time.sleep(Duration)

        BluetoothDialogInput('Press Enter Then verify the FTP connection by refreshing (Windows F5) the file system on the platform device')

        # wait for the user to refresh the FTP file system on the platform device.
        self.LogcatWait(ComponentOne, 'Accepted connection from %s'%FTPAddress, 45)
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_12_28_FTP_Long_Duration_Idle_All_1("TC_AD_Bluetooth_12_28_FTP_Long_Duration_Idle_All_1", duts)
        tc.run()
