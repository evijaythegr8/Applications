...
class TC_AD_Bluetooth_12_19_FTP_Pull_Large_File_All_1(BluetoothTestCase):

    def execute(self):
        ComponentOne = '1'
        PTSDongleAddress = Session.getSetupFileParam('bt', 'addresspts')
        ptsTestCase = 'TC_SERVER_OTR_BV_13_I'
        ...
        # Create extremly large file
        self.RunShellCommand(ComponentOne, 'dd if=/dev/zero of=/sdcard/EXT-LARGE.txt count=50000')
        time.sleep(20)
        ...
        self.WaitForEvent(ComponentOne, OpCodes.ACTION_ACL_DISCONNECTED, timeout=2400)

        #Clean up file
        self.RunShellCommand(ComponentOne, 'rm /sdcard/EXT-LARGE.txt')
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_12_19_FTP_Pull_Large_File_All_1("TC_AD_Bluetooth_12_19_FTP_Pull_Large_File_All_1", duts)
        tc.run()
