...
class TC_AD_Bluetooth_7_05_OPP_Incoming_Transfer_Single_Small_File_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        self.RunShellCommand(ComponentOne, "rm /sdcard/bluetooth/*.vcf")
        ...
        self.CreateContact(ComponentTwo, contactName, contactNumber, contactMail)
        time.sleep(2)
        self.Wakeup(ComponentTwo, timeout=20)
        self.SendContactListViaOPP(ComponentTwo)

        #Wait for incoming toast noting an incoming transfer request then accept it
        self.WaitForEvent(ComponentOne, OpCodes.ACTION_ACL_CONNECTED, timeout=40)
        self.Wakeup(ComponentOne, timeout=20)
        self.AcceptIncomingNotification(ComponentOne)

        #Wait for transfer to finish
        self.WaitForEvent(ComponentOne, OpCodes.ACTION_ACL_DISCONNECTED, timeout=40)
        time.sleep(2)

        #Import contacts
        self.Wakeup(ComponentOne, timeout=10)
        self.ImportSingleContact(ComponentOne)
        time.sleep(5)

        #Retreive contacts
        self.SendCommand(ComponentOne, OpCodes.GET_CONTACTS, message='Getting contacts')
        foundContacts = []
        foundContacts = self.WaitForEventAndStore(ComponentOne, OpCodes.NO_MORE_CONTACTS, OpCodes.CONTACT_FOUND, timeout=120)
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_7_05_OPP_Incoming_Transfer_Single_Small_File_All_1("TC_AD_Bluetooth_7_05_OPP_Incoming_Transfer_Single_Small_File_All_1", duts)
        tc.run()
