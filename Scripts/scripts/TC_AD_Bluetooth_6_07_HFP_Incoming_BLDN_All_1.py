...
class TC_AD_Bluetooth_6_07_HFP_Incoming_BLDN_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        time.sleep(2)
        BluetoothDialogInput("Press OK, then initiate a BLDN (last dialed number) from the handsfree")

        #Check for BLDN in logs
        self.log.info("------ Running Shell Command Start ------")
        outData = self.RunShellCommandAndHciDump(ComponentOne, 'date')
        if find(outData,'BLDN') != -1:
            self.log.info("Found correct event")
        else:
            raise Failure("AT+BLDN not found in log. Are you using Headset with BLDN?")
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_6_07_HFP_Incoming_BLDN_All_1("TC_AD_Bluetooth_6_07_HFP_Incoming_BLDN_All_1", duts)
        tc.run()
