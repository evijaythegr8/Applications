...
class TC_AD_Bluetooth_6_23_HFP_Disconnect_HFP_From_HF_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        BluetoothDialogInput("Press Enter then turn off the headset")

        self.WaitForEvent(ComponentOne, OpCodes.ACTION_ACL_DISCONNECTED, timeout=45)

        time.sleep(10)

        BluetoothDialogInput("Turn on the headset then press Enter")

        BluetoothDialogInput("Connect headset by pressing the multi-function button on the headset then press Enter")
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_6_23_HFP_Disconnect_HFP_From_HF_All_1("TC_AD_Bluetooth_6_23_HFP_Disconnect_HFP_From_HF_All_1", duts)
        tc.run()
