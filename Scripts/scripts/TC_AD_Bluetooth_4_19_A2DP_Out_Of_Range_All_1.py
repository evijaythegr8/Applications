...
class TC_AD_Bluetooth_4_19_A2DP_Out_Of_Range_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        BluetoothDialogInput('Press OK and then walk out of range with the headset. When connection is lost wait for a while then return back')
        self.WaitForEvent(ComponentOne, OpCodes.ACTION_ACL_DISCONNECTED, timeout=160)
        time.sleep(5)
        BluetoothDialogInput('Press the play button. then press Enter')

        #Check A2DP connected
        self.WaitForEvent(ComponentOne, OpCodes.CURR_SINK_STATE_CONNECTED, timeout=30)
        ...
        if sinkState != BTC.STATE_CONNECTED:
            ...
            raise Failure('A2DP not connected')
        time.sleep(2)
        BluetoothDialogInput('Verify that audio is playing in the A2DP headset. Yes/No',option='TEST')
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_4_19_A2DP_Out_Of_Range_All_1("TC_AD_Bluetooth_4_19_A2DP_Out_Of_Range_All_1", duts)
        tc.run()
