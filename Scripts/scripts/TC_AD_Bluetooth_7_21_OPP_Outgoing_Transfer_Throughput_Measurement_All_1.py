...
class TC_AD_Bluetooth_7_21_OPP_Outgoing_Transfer_Throughput_Measurement_All_1(BluetoothTestCase):

    def execute(self):
        ComponentOne = '1'
        ComponentTwo = '2'
        image = 'pic_18.0_Mpix_NoName_3x2_JFIF_1.02_Exif_2.21_BaselineStd_Q11.jpg'
        imagesizebytes = 6602251
        mediaPath = '/sdcard/%s'%image
        targetPath = '/sdcard/bluetooth/%s'%image
        requirement = float(Session.getSetupFileParam('bt', 'oppthroughput'))
        ...
        #Start measurement when Transfer has started
        starttime = time.time()
        ...
        imagesizeMbits = float(imagesizebytes)*8.0/1000000
        tp = float()
        tp = (imagesizeMbits)/(transferTime)
        time.sleep(3)
        print("ThroughPut = %s, tt = %s, da = %s, da/tt = %s"%(tp, transferTime, imagesizeMbits,(imagesizeMbits/transferTime)))
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_7_21_OPP_Outgoing_Transfer_Throughput_Measurement_All_1("TC_AD_Bluetooth_7_21_OPP_Outgoing_Transfer_Throughput_Measurement_All_1", duts)
        tc.run()
