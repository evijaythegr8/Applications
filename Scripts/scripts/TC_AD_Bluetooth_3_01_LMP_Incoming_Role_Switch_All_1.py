...
class TC_AD_Bluetooth_3_01_LMP_Incoming_Role_Switch_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        if find(outData,'> HCI Event: Role Change (0x12) plen 8') != -1 and find(outData,' role 0x01') != -1 and find(outData,'Role: Slave') != -1:
            ...
            raise Failure("Faild to set role slave")
        self.log.info("------ Running Shell Command Done ------")
        time.sleep(5)
        ...
        if find(outData,'> HCI Event: Role Change (0x12) plen 8') != -1 and find(outData,' role 0x00') != -1 and find(outData,'Role: Master') != -1:
            ...
            raise Failure("Faild to set role master")
        self.log.info("------ Running Shell Command Done ------")
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_3_01_LMP_Incoming_Role_Switch_All_1("TC_AD_Bluetooth_3_01_LMP_Incoming_Role_Switch_All_1", duts)
        tc.run()
