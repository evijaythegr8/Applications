...
class TC_AD_Bluetooth_9_15_SPP_Data_Transfer_Long_Duration_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        #Start data transfer
        self.log.info("START transfer, time = %s"%(time.localtime()))
        loopcounter = 1
        stopTime = time.time() + Duration*60
        self.SendCommand(ComponentTwo, OpCodes.RECEIVE_DATA_CLIENT, message='%s;%s;%s'%(clientId, BTC.RFCOMM_CHECK_DATA, bufferSize))
        while time.time() < stopTime:
            self.log.info("Loopcounter", "Current iter: %s , time left: %s"%(loopcounter, stopTime-time.time()))
            time.sleep(1)
            self.SendCommand(ComponentOne, OpCodes.DATA_PUMP_SERVER, message='%s;%s;%s'%(serverId, DataAmountKb*1024, bufferSize))
            self.WaitForEvent(ComponentOne, OpCodes.DATA_PUMP_SERVER, timeout=240)
            self.log.info("Timestamp: %s"%time.localtime())
            loopcounter = loopcounter+1
        self.log.info("END transfer, time = %s"%(time.localtime()))
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_9_15_SPP_Data_Transfer_Long_Duration_All_1("TC_AD_Bluetooth_9_15_SPP_Data_Transfer_Long_Duration_All_1", duts)
        tc.run()
