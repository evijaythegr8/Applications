...
class TC_AD_Bluetooth_1_36_GAP_Consecutive_Device_Discovery_All_1(BluetoothTestCase):
    ...
    def execute(self):
        ...
        for i in range(0,iterations):
            ...
            #Unset discoverable (by setting it to 1s)
            self.Wakeup(ComponentTwo, timeout=10)
            self.SendAndWaitForEvent(ComponentTwo, OpCodes.SET_DISCOVERABLE, OpCodes.SET_DISCOVERABLE, message='1', timeout=10)
            time.sleep(6)
            self.pressYes(ComponentTwo)
            time.sleep(5)
...
if __name__ == '__main__':
    ...
    if(env.setup()):
        tc = TC_AD_Bluetooth_1_36_GAP_Consecutive_Device_Discovery_All_1("TC_AD_Bluetooth_1_36_GAP_Consecutive_Device_Discovery_All_1", duts)
        tc.run()
